...
- **No remote code is executed fresh at runtime, and every shipped script is hash-pinned.** All files in `agent_reach/scripts/` are checksummed in `agent_reach/scripts/CHECKSUMS.sha256`; the installer verifies a script's sha256 against that manifest before running/copying it and refuses on mismatch (`_verify_vendored_script`). A test fails if any script changes/adds/removes without a re-pin. The NodeSource apt-repo setup script is the vendored third-party case (`_NODESOURCE_SETUP_SHA256`); `agent-reach verify-node-script` (+ the weekly `nodesource-drift` CI job) re-fetch upstream and flag drift so we re-review + re-pin deliberately.
