...
- **No remote code is executed fresh at runtime.** The NodeSource Node.js apt-repo setup script is **vendored** at `agent_reach/scripts/nodesource_setup_22.x.sh` and **sha256-pinned** (`_NODESOURCE_SETUP_SHA256`); the installer verifies the hash before running our reviewed copy and refuses on mismatch. `agent-reach verify-node-script` (+ the weekly `nodesource-drift` CI job) re-fetch upstream and flag drift so we re-review + re-pin deliberately.
